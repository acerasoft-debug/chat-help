...
<div class="wrap">
  <div class="phead" style="margin-bottom:26px">
    ...
    <h1 style="display:flex;align-items:center;gap:12px;flex-wrap:wrap"><?= htmlspecialchars($dispName) ?>
      <?php if ($verified): ?><span class="gal-vbadge" style="position:static"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg> <?= t('Verified seller') ?></span><?php endif; ?>
    </h1>
      ...
      <a class="scard" href="/product?id=<?= urlencode($p['id']) ?>">
        <div class="sthumb" style="background:linear-gradient(135deg,<?= htmlspecialchars(vestra_accent($p)) ?>,#0e0e11)">
          ...
          <?php if (!empty($p['verified'])): ?>
            <span class="svbadge"><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg> <?= t('Verified seller') ?></span>
          <?php endif; ?>
