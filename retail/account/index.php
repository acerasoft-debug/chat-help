...
$spent  = 0;
foreach ($orders as $o) $spent += (int)($o['total_cents'] ?? 0);

vr_layout_start(['title' => t('acc_title'), 'robots' => 'noindex,nofollow']);
?>
<section class="sec">
  <div class="wrap">
    <h1 class="sechead__t" style="margin-bottom:28px"><?= te('acc_title') ?></h1>
    <div class="dash">
      <?php vr_accnav('index.php'); ?>
      <div>
        <?php vr_verify_notice($me); ?>

        <p class="lede" style="margin-bottom:22px"><?= te('acc_hello', ['name' => h(trim((string)$me['name']) !== '' ? $me['name'] : $me['email'])]) ?></p>

        <div class="stats">
          <div><b><?= count($orders) ?></b><span><?= te('acc_stat_orders') ?></span></div>
          <div><b><?= h(vr_money($spent)) ?></b><span><?= te('acc_stat_spent') ?></span></div>
          <div><b><?= h(vr_date((int)($me['created_at'] ?? time()))) ?></b><span><?= te('acc_stat_since') ?></span></div>
        </div>

        <h2 class="sechead__t" style="font-size:22px;margin:34px 0 14px"><?= te('acc_recent') ?></h2>
        <?php if (!$orders): ?>
