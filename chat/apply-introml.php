<?php
/**
 * ChatHelp — apply-introml (CH_INTRO_ML) — giris intro'sunu COK DILLI yapar.
 *   [1] Eski Almanca-sabit CH_INTRO'yu devre disi birakir (bir 'return;').
 *   [2] Ayni premium tasarimi (lacivert+altin) koruyup metinleri ch_uilang'e gore
 *       de/en/tr/fr/es/it sozlugunden secen yeni intro'yu </body>'den once enjekte eder.
 *   Additive + fail-open + idempotent + php -l lint + .bak + opcache + dogrulama.
 * KULLANIM: /chat/pull2.php?key=...&files=apply-introml.php
 */
header('Content-Type: text/plain; charset=UTF-8');
error_reporting(E_ERROR | E_PARSE);
echo "apply-introml BASLADI OK (PHP ".PHP_VERSION.")\n\n";
$file=__DIR__.'/index.php';
$src=@file_get_contents($file);
if($src===false) exit("index.php okunamadi\n");
if(strpos($src,'CH_INTRO_ML')!==false) exit("Zaten ekli (CH_INTRO_ML).\n");

/* ── [1] eski CH_INTRO'yu devre disi birak ── */
$oldA="var KEY='ch_intro_story_v1';";
$oldN="var KEY='ch_intro_story_v1';return;/*CH_INTRO_ML — eski tek-dil intro kapatildi*/";
$co=substr_count($src,$oldA);
if($co===1){ $src=str_replace($oldA,$oldN,$src); echo "  ✓ eski CH_INTRO devre disi (1)\n"; }
elseif($co===0){ echo "  · eski CH_INTRO bulunamadi — sadece yeni enjekte edilecek\n"; }
else { exit("  ✗ eski intro anchor $co kez — guvenlik icin DEGISTIRILMEDI\n"); }

/* ── [2] cok dilli yeni intro ── */
$block=<<<'BLOCK'
<script>/*CH_INTRO_ML — cok dilli giris intro'su (bir kez, fail-open)*/(function(){try{
  ...
  function build(){
    ...
    var html=""
    +"<div class='chi2-p' role='dialog' aria-label='ChatHelp'>"
...
$tmp=tempnam(sys_get_temp_dir(),'iml').'.php'; file_put_contents($tmp,$new);
$lo=[];$rc=0; exec('php -l '.escapeshellarg($tmp).' 2>&1',$lo,$rc); @unlink($tmp);
if($rc!==0){ echo "  ✗ LINT HATASI — DEGISTIRILMEDI:\n  ".implode("\n  ",$lo)."\n"; exit; }

@file_put_contents($file.'.bak-introml-'.date('Ymd-His'),(string)@file_get_contents($file));
$w=@file_put_contents($file,$new);
if($w===false||$w<strlen($new)) exit("  ✗ YAZMA HATASI.\n");
if(function_exists('opcache_reset')) @opcache_reset();
$chk=(string)@file_get_contents($file);
if(strpos($chk,'CH_INTRO_ML')===false) exit("  ✗ DOGRULAMA BASARISIZ.\n");
echo "\n  ✓ CH_INTRO_ML enjekte edildi (".strlen($chk)." B)\n";
echo "  · Intro artik ch_uilang'e gore de/en/tr/fr/es/it cikar.\n";
echo "  · Bir kez oynar (ch_intro_ml_v1). Tekrar gormek: bu anahtari temizle.\n";
