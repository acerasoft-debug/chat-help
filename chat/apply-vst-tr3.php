 ...
 * KULLANIM: pull2.php?key=...&files=apply-vst-tr3.php
 */
...
if (strpos($src,'ncxBuild')===false) {
    echo "HATA: vst-more (ncxBuild) yok — once apply-vst-more3.\n";
    echo "  teshis: index ".strlen($src)." bayt, CH_VST_MORE:".substr_count($src,'CH_VST_MORE')
        .", bos alan ".round(@disk_free_space(__DIR__)/1048576,1)." MB\n";
    exit;
}
...
$bk=@file_put_contents($file.'.bak-vsttr-'.date('Ymd-His'), @file_get_contents($file));
if ($bk===false) echo "  ⚠ yedek yazilamadi (disk dolu olabilir) — devam ediliyor\n";
$w=@file_put_contents($file,$src);
if ($w===false || $w<strlen($src)) {
    echo "\n✗ YAZMA HATASI: donen ".var_export($w,true)." / beklenen ".strlen($src)." bayt.\n";
    echo "  bos alan: ".round(@disk_free_space(__DIR__)/1048576,1)." MB — once apply-bak-cleanup calistir.\n";
    exit;
}
$chk=(string)@file_get_contents($file);
if (strpos($chk,'CH_VST_TR')===false) { echo "\n✗ DOGRULAMA BASARISIZ: yazim sonrasi CH_VST_TR diskte YOK.\n"; exit; }
echo "  ✓ DOGRULAMA: CH_VST_TR diskte (".strlen($chk)." bayt)\n";
