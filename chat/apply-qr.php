<?php
/**
 * ChatHelp — apply-qr (CH_QR_APP) — Google Play QR widget (sol alt, kapatilabilir).
 *  EKLEMELI </body>: sol-altta kucuk bir "Android App" pili; tiklayinca QR +
 *  Google Play butonu acilir. QR gomulu (data-URI, harici baglanti yok).
 *  Kalici kapatma (x) localStorage'da tutulur.
 * KULLANIM: pull2.php?key=...&files=apply-qr.php
 */
header('Content-Type: text/plain; charset=UTF-8');
error_reporting(E_ERROR | E_PARSE);
echo "apply-qr BASLADI OK (PHP ".PHP_VERSION.")\n\n";
$file = __DIR__.'/index.php';
$src = @file_get_contents($file);
if ($src===false) exit("index.php okunamadi\n");
if (strpos($src,'CH_QR_APP')!==false) exit("Zaten ekli (CH_QR_APP).\n");
...
@file_put_contents($file.'.bak-qr-'.date('Ymd-His'), $src);
$w=@file_put_contents($file,$new);
if ($w===false || $w<strlen($new)) { echo "\n✗ YAZMA HATASI.\n"; exit; }
$chk=(string)@file_get_contents($file);
if (strpos($chk,'CH_QR_APP')===false) { echo "\n✗ DOGRULAMA BASARISIZ.\n"; exit; }
echo "  ✓ DOGRULAMA: CH_QR_APP diskte (".strlen($chk)." bayt)\n";
echo "\n✓ Sol altta 'Android App' pili — tiklayinca Google Play QR + buton. Gomulu QR.\n";
