<?php
/**
 * ChatHelp — apply-install-pull2 — GUVENLI KANALA GECIS (tek adim)
 *  [1] Rastgele gizli anahtar uretir -> pull-key.php'ye yazar (git'te YOK,
 *      yalniz bu sunucuda; ciktida BIR KEZ gosterilir — KAYDET!)
 *  [2] pull2.php'yi (anahtar korumali yeni kanal) kurar.
 *  [3] ESKI ACIK KANALI KAPATIR: pull-updates.php silinir. Boylece sunucuda
 *      kimlik dogrulamasiz "uzaktan kod indir-calistir" ucu KALMAZ —
 *      GoDaddy guvenlik tarayicilarinin webshell suphesi kokten biter.
 * KULLANIM (eski kanaldan SON istek): pull-updates.php?files=apply-install-pull2.php
 */
header('Content-Type: text/plain; charset=UTF-8');
error_reporting(E_ERROR | E_PARSE);
$D = __DIR__;
echo "apply-install-pull2 BASLADI OK (PHP ".PHP_VERSION.")\n\n";

/* [1] anahtar */
$kf = $D.'/pull-key.php';
if (is_file($kf)) {
    $key = (string)@include($kf);
    echo "  ✓ [1] mevcut anahtar korunuyor (daha once kurulmus)\n";
} else {
    $key = bin2hex(random_bytes(16));
    if (file_put_contents($kf, "<?php return '".$key."';\n") === false) exit("HATA: pull-key.php yazilamadi.\n");
    echo "  ✓ [1] yeni gizli anahtar uretildi ve pull-key.php'ye yazildi\n";
}

/* [2] pull2.php govdesi (pull2.php ile ayni icerik, gomulu) */
$p2 = <<<'P2SRC'
<?php
/**
...
file_put_contents($tmp,$p2);
$lo=[];$rc=0; exec('php -l '.escapeshellarg($tmp).' 2>&1',$lo,$rc); @unlink($tmp);
if ($rc!==0) { echo "\nLINT HATASI — pull2 KURULMADI:\n  ".implode("\n  ",$lo)."\n"; exit; }
if (file_put_contents($D.'/pull2.php', $p2) === false) exit("HATA: pull2.php yazilamadi.\n");
echo "  ✓ [2] pull2.php kuruldu (anahtar korumali kanal)\n";

/* [3] eski acik kanali kapat */
if (is_file($D.'/pull-updates.php')) {
    echo (@unlink($D.'/pull-updates.php')
        ? "  ✓ [3] ESKI ACIK KANAL KAPATILDI: pull-updates.php silindi\n"
        : "  ⚠ [3] pull-updates.php silinemedi — GoDaddy dosya yoneticisinden elle sil!\n");
} else {
    echo "  ✓ [3] pull-updates.php zaten yok\n";
}

echo "\n══════════════════════════════════════════════════\n";
echo "  YENI GIZLI ANAHTARIN (BIR KEZ gosteriliyor — KAYDET):\n\n";
echo "      $key\n\n";
echo "  Yeni kullanim:\n";
echo "  https://chat-help.com/chat/pull2.php?key=$key&files=apply-x.php\n";
echo "  (files'siz cagrida: saglik raporu)\n";
echo "══════════════════════════════════════════════════\n";
