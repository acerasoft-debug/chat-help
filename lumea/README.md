...
pass identity + document verification, and accept nearby requests. Three languages, twenty cities,
twenty-three treatments — from anti-cellulite and lymphatic drainage to HIFU lifting and signature facials.
...
| Static generator | `scripts/build.mjs`, `src/lib/` | 1 179 pages, sitemap, robots, manifest, JSON-LD, hreflang |
...
Every page carries `canonical`, `hreflang` (de/en/es/x-default), Open Graph/Twitter cards and typed JSON-LD
...
- **Language:** add the code to `site.locales`, a `t.<code>` block in `data/i18n.mjs`, a
  `services.<code>.mjs`, URL segments in `src/lib/html.mjs`, and journal/Privé/legal copy. French and Italian for
  Switzerland are the obvious next candidates.
